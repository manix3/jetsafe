...
function modalData(idr) {
  ...
  $.post(base_url+ `Profesionales/get_profesional/${idr}`,function(data) {
    var detalle = '';
    var id_cat = '';
    $.each(data,function (i,item) {
      $("#inp_text1").val(item.ide_profecional)
      $("#inp_text2").val(item.nom_prof)
      $("#inp_text3").val(item.tfl_prof)
      ...
      detalle += `
      <tr><th>Nombre:</th><td>${item.nom_prof}</td></tr>
      <tr><th>Telefono:</th><td>${item.tfl_prof}</td></tr>
  ...
  function jalar_data() {
    datatable = ''
    $.getJSON(base_url+'Profesionales/list_profesionales', function(data){
      $.each(data, function (i,item) {
        datatable += `
        <tr>
          <td>${item.nom_prof}</td>
          <td>${item.tfl_prof}</td>
          <td class="ver_registro" idr="${item.ide_profecional}" ><i class="fa fa-search"></i> </td>
          <td class="edi_registro" idr="${item.ide_profecional}"><i class="fa fa-pencil"></i> </td>
          <td class="bor_registro" idr="${item.ide_profecional}"><i class="fa fa-trash-o"></i> </td>
        </tr>
                    `;
...
let accion = $("#accion").val()
  ...
  $.ajax({
    url:`${base_url}/Profesionales/${destino}`,
