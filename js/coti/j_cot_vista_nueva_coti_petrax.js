...
*/
$('#nom_ven').change(function () {
    let id = $(this).val()
    var vend,coti;
    vend = {
      inp_ape_pat_ven: null,
      inp_ape_mat_ven: null,
      doc_ven: null,
      mail_ven: null,
      ruc_ven: null,
      dir_ven: null,
...
/*
  $('#nom_ven').change(function () {
      let id = $(this).val()
      var vend,coti;
      $.getJSON(Gruta+'coti/cot_vendedor/ven_list/'+id, function (data) {
        if (data.length === 1) {
          $.each(data, function (i,item) {

            $('#inp_ape_pat_ven').val(item.ven_ape_pat)
            $('#inp_ape_mat_ven').val(item.ven_ape_mat)
            $('#doc_ven').val(item.ven_doc)
            $('#mail_ven').val(item.ven_email)
            $('#ruc_ven').val(item.ven_ruc)
            $('#dir_ven').val(item.ven_dir)
            $('#inp_tel_ven1').val(item.ven_tel1)
            $('#inp_tel_ven2').val(item.ven_tel2)


            vend = {
              inp_ape_pat_ven: item.ven_ape_pat,
              inp_ape_mat_ven: item.ven_ape_mat,
              doc_ven: item.ven_doc,
              mail_ven: item.ven_email,
              ruc_ven: item.ven_ruc,
              dir_ven: item.ven_dir,
              inp_tel_ven1: item.ven_tel1,
              inp_tel_ven2: item.ven_tel2,
              ide_vendedor : id,
            }
...
/*WORKSPACE*/
    ...
    if (form.parsley().isValid()){
      $.ajax({
        ...
        success:function(data){
          comboselect(data,Gruta+'public/Clientes/cli_listar_petrax_seg','Seleccione Cliente','item.ide_cliente','item.nomdoc','form_cli','nom_cli');
  ...
  $('#do').click(function () {
    ...
    if (typeof lista !== "undefined") {
      ...
      let   cont = {
        inp_nom_con: $('#inp_nom_con').val() ,
        inp_fun_con: $('#inp_fun_con').val() ,
        inp_tel_con1: $('#inp_tel_con1').val() ,
        inp_tel_con2: $('#inp_tel_con2').val() ,
        inp_ema_con: $('#inp_ema_con').val() ,
        ide_tip_moneda: $('#ide_tip_moneda').val() ,
        ide_ori_cot: $('#ide_ori_cot').val() ,
        est_coti: $('#est_coti').val() ,
      }

      lista.coti.push(cont)

          console.log(lista);
          $.ajax({
            url:Gruta+'coti/Cot_cotizacion/nueva_cotizacion_ins_petrax',
            type:'post',
            data:lista,
            success: function(data) {
              aa = data;
              window.location=Gruta+"Cotizacion/cotizar"; // Agregado por Orlando hay que borrar y darle la opcion de enviar correo o imprimir PDF
            },
...
function pjaxPageLoad(){
  ...
  comboselect(null,Gruta+'public/Clientes/cli_listar_petrax_seg','Seleccione Cliente','item.ide_cliente','item.nomdoc','form_cli','nom_cli');
  comboselect(null,Gruta+'coti/cot_vendedor/ven_list','Seleccione Vendedor','item.ide_vendedor','item.ven_nom','form_ven','nom_ven');
  comboselect(1,Gruta+'coti/Cot_cotizacion/get_ori_estado','Seleccione Origen','item.ide_ori_cot','item.detalle_ori_cot','form_coti','ide_ori_cot');
