class Lista {
  constructor(cliente, coti, ide_cotizacion) {
    this.ide_cotizacion = ide_cotizacion;
  ...
  actualizar(id_prod,cat){



    $.each(this.productos,function (i,item) {
      console.log(id_prod,cat);
      console.log(item.ide_producto);
      if (item.ide_producto == id_prod) {
          item.cot_cantidad = cat;
          // item.cot_precio = parseFloat(item.cot_precio) * cat;
      }
 ...
 remover(ind){
   this._subTotal = this._subTotal - (this.productos[ind].cot_precio * this.productos[ind].cot_cantidad);
   this.igv();
   this.total();
...
$(function(){
  ...
  $('#ide_tip_moneda').change(function () {
    let id = $(this).val()
      $.getJSON(Gruta+'coti/Cot_mantenimiento/tip_Mon_listar/'+coti.ide_tip_mon, function (data) {
          if (data.length === 1) {
  ...
  $(document).ready(function () {
      var cli,cotis;
            ...
            cli = {
              email_destino:$('#mail').val(),
              nombre_destino:$('#nom_cli').val()  ,
              telefono_destino:$('#inp_tel_cli1').val(),
              cli_ruc:$('#ruc').val(),
              cli_dir:$('#dir').val()  ,
            }
          ...
          cotis = {
            ...
            lista= new Lista(cli,cotis, ide_cotu);
                    ...
                    <div class="col-lg-4">
                      ...
                      <div class="input-group">
                            <span class="input-group-addon"><i class="fa fa-cubes"></i></span>
                              <input class="form-control"  min="1" type="number" placeholder="Cantidad" value="${cant}" >
                          </div>
  ...
  $('#do').click(function () {
    if (typeof lista !== "undefined") {
      if (lista.productos.length !== 0) {
          console.log(lista);
          $.ajax({
            url:Gruta+'factu/Facturar/edita_cotizacion_upd',
            type:'post',
            data:lista,
            success: function(data) {
              // aa = data;
              window.open(Gruta+'f_files/'+data.nombre);
              window.location=Gruta+"Factura"; // Agregado por Orlando hay que borrar y darle la opcion de enviar correo o imprimir PDF
            },
