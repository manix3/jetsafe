...
//alert($('#vendors').val());
    ...
    glob= q;
    $('#inp_text1').val(q);
    console.log(Gruta+'coti/Cot_cotizacion/seg_listar/'+q);
    $.getJSON(Gruta+'coti/Cot_cotizacion/seg_listar/'+q,function(data) {
      var cli_list='';
      $.each(data,function (i,item) {
        cli_list+=`
...
/*** Fin ver registro ****/
    ...
    $.getJSON(Gruta+'coti/Cot_cotizacion/seg_listar/'+idr, function(data) { // Inicio JSON
...
//        $('#inp_text2').val(item.det_est_cot);
    ...
    $("#datatable-table").on('click','.bor_registro',function() {
      ...
      $.getJSON(Gruta+'coti/Cot_cotizacion/seg_list_busqu/'+idr, function(data) { // Inicio JSON
        $.each(data, function(k,item){
...
///  $('#inp_text4').val('');
  ...
  function jalar_data(off=0, ide_vendedor = null ) {
  //  alert('xxx');
    ...
    $('#datatable-table').dataTable().fnDestroy();
    $.post(Gruta+'coti/Cot_cotizacion/seg_list_busqu',{offset:off, ide_vendedor: ide_vendedor },function(data) {
...
$('#datatable-table').removeClass('d-none');
      ...
      tabledatos = $('#datatable-table').DataTable({
        ...
        "aoColumnDefs": [
          ...
          { 'bVisible': false, 'aTargets': [ 4,6,7,8] }
...
function more() {
  offset+=500;
  jalar_data(offset,ses_vendedor)
}
