...
function modalData(idr) {
  ...
  $.post(base_url+ `Mano_de_obra/get_man_obra/${idr}`,function(data) {
    var detalle = '';
    var id_cat = '';
    $.each(data,function (i,item) {
      $("#inp_text1").val(item.ide_man)
      $("#inp_text2").val(item.nom_man)
    id_cat = item.ide_categoria
      ...
      detalle += `
      <tr><th>Nombre:</th><td>${item.nom_man}</td></tr>
  ...
  function jalar_data() {
    datatable = ''
    $.getJSON(base_url+'Mano_de_obra/list_man_de_obra', function(data){
      $.each(data, function (i,item) {
        datatable += `
        <tr>
          <td>${item.nom_man}</td>
          <td>${item.nom_cat}</td>
          <td class="ver_registro" idr="${item.ide_man}" ><i class="fa fa-search"></i> </td>
          <td class="edi_registro" idr="${item.ide_man}"><i class="fa fa-pencil"></i> </td>
          <td class="bor_registro" idr="${item.ide_man}"><i class="fa fa-trash-o"></i> </td>
        </tr>
                    `;
...
let accion = $("#accion").val()
  ...
  $.ajax({
    url:`${base_url}/Mano_de_obra/${destino}`,
