...
function modalData(idr) {
  ...
  $.post(base_url+ `Categorias/get_cat/${idr}`,function(data) {
    var detalle = '';
    $.each(data,function (i,item) {
      ...
      detalle += `
      <tr><th>Nombre:</th><td>${item.nom_cat}</td></tr>
  ...
  function jalar_data() {
    datatable = ''
    $.getJSON(base_url+'Categorias/list_categorias', function(data){
      $.each(data, function (i,item) {
        datatable += `
        <tr>
          <td>${item.nom_cat}</td>
...
let accion = $("#accion").val()
  ...
  $.ajax({
    url:`${base_url}Categorias/${destino}`,
